...
- A data frame can be created using *data.frame()*, which takes named vectors as input.


```{r}
yawn_expt <- data_frame(group = c(rep("control", 16), 
                                  rep("treatment", 34)),
...
- **%>%**: the "pipe" operator is used to connect multiple dplyr functions in a sequence of operations.



```{r eval = FALSE}
 third(second(first(x))) #Difficult to read a sequence of operations
```
...
ggplot(data = economics, aes(x = pce)) +
  geom_histogram(binwidth = 500,  col="blue", fill="lightblue")+

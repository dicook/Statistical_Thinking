...
a. Is the grey area greater than 0.5? YES or NO
b. Which letter represents a quantile? X or Y

2. Which of the following would be the density function for a Poisson distribution? (circle it)

$$f(x~|~\lambda, k) = \frac{k}{\lambda}\left(\frac{x}{\lambda}\right)^{k-1} e^{(-x/\lambda)^k}, ~~~ x\geq 0 ~~~ or ~~~ P(X=x~|~\lambda) = \frac{\lambda^x e^{-\lambda}}{x!} ~~ x \in \{0, 1, 2, ...\}$$

...
- Most children start at age 5 or 6 in Australia, there is less data for 4 and 7 years olds, but probably ok
- Students in Australia don't spend much time on homework. Beyond 3 hours per week the numbers dwindle dramatically. It will be necessary to combine groups, maybe 9 or more hours into one group. Most data is missing on this variable, though. So it might be better to just not use it for modeling.
- Mothers full-time, part-time and at-home are large groups, number of mothers looking for work is low, but migt be ok
- Fathers are mostly in full-time work, numers in all other categories are relatively low.
- Most Australian 15 years olds have a desk, their own room, computer and internet! Numbers without these are low, but maybe still enough for modeling.
...
There is a lot of variation in the weights. The weights are bimodal (is the bmodality due to one of the variables in the study that we are using for the model?) with a few very large ones. It looks like we will need to take weight into account in the model.
...
- What value would be considered to tbe the cutoff for considering a case to have high leverage?
